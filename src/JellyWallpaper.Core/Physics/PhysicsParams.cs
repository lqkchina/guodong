...
public sealed class PhysicsParams
{
    ...
    /// 建议范围 12 ~ 120，默认 48（果冻预设：网格稍疏 → 形变更整体、更柔和）。
    ...
    /// 果冻预设 60：回弹偏"糯"、形变深，Q 弹感明显。
    /// </summary>
    public double Stiffness { get; set; } = 60.0;
    ...
    ///   ζ = 1   → 临界阻尼，最快回到静止且不震荡；
    ///   ζ > 1   → 过阻尼，缓慢爬回静止，无弹感。
    /// 果冻预设 0.08：轻微阻尼 → 松手后明显"颤两下再停"。
    /// </summary>
    public double Damping { get; set; } = 0.08;

    /// <summary>
    /// 拖拽影响半径（像素）：鼠标周围多大范围内的质点会被拖拽力影响。
    /// 半径之外的质点完全不受鼠标影响，仅受弹簧网络牵动。
    /// 果冻预设 300：拉扯范围大 → 形变"整体"，不是只动一小块。
    /// </summary>
    public double DragRadius { get; set; } = 300.0;

    /// <summary>
    /// 拖拽强度系数（0 ~ 3）。控制拖拽外力的比例系数：
    /// k_drag = 2 · DragStrength · Stiffness。
    /// 越大，质点"跟随鼠标"越紧密。果冻预设 1.0。
    /// </summary>
    public double DragStrength { get; set; } = 1.0;

    /// <summary>
    /// 质点最大位移限制（像素）：|P_current − P_rest| 的上限，
    /// 防止网格被拖到极端扭曲甚至撕裂。果冻预设 240：形变幅度大。
    /// </summary>
