...
";
                                ...
                                float cellSize, int screenW, int screenH, int texW, int texH)
    {
        ...
        // v5.37：壁纸跨区（Cover）显示 —— 大图等比缩放铺满整个屏幕。
        // 与系统"填充/跨区"语义一致：取壁纸上等比缩放到屏幕大小的居中区域
        // （scale = min(texW/screenW, texH/screenH)），整张图完整可见，
        // 不再 1:1 只显示左上角裁剪部分。
        // 壁纸缺失/加载失败时（texW<=0）回退 1:1 兜底（等价原逻辑）。
        ...
        for (int r = 0; r < rows; r++)
        {
            for (int c = 0; c < cols; c++)
            {
                ...
                // UV = 未形变网格坐标 → 壁纸源区域（Cover 跨区映射）
                _vertexData[vi + 2] = u0 + c * cellSize * uScale;
                _vertexData[vi + 3] = v0 + r * cellSize * vScale;
            }
        }
